...
Все сервисы подключены к кастомной изолированной сети `ae-project-network`:

- **Тип сети**: Bridge (мостовая)
- **Подсеть**: `172.25.0.0/16` 
- **Изоляция**: Полная изоляция от других Docker сетей
- **Внутренняя связь**: Сервисы общаются по именам контейнеров
  - Backend → PostgreSQL: `postgres:5432`
  - Frontend → Backend: `backend:8000`

#### Преимущества изолированной сети:
- **Безопасность**: Сервисы недоступны извне без проброса портов
- **Производительность**: Быстрая внутренняя связь между контейнерами
- **Изоляция**: Конфликты с другими проектами исключены
- **Управление**: Централизованная настройка DNS и маршрутизации
